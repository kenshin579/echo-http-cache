...
1. 이슈 번호 또는 브랜치: 
   - GitHub 이슈 번호가 있는 경우: 대괄호 안에 이슈 번호를 작성합니다 (예: `[#3000]`)
   - 이슈 번호가 없는 경우: 대괄호 안에 브랜치명을 작성합니다 (예: `[feat/implement-stock-api]`)
...
### 이슈 번호가 없는 경우 (브랜치명 사용)
```
[feat/stock-price-api] 주식 가격 조회 API 초기 구현
...
```
[hotfix/db-connection] 데이터베이스 연결 오류 긴급 수정
